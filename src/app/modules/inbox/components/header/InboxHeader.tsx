...
const InboxHeader = ({
  ...
  useEffect(() => {
    if (clientIdForFilter) {
      setClientName(
        JSON.parse(localStorage.getItem("storedClientForInbox")!)?.displayName
      );
    }
  }, [clientIdForFilter]);
  ...
  useEffect(() => {
    if (year || statusTypes || clientIdForFilter || periodValueType) {
      setIsFilterApplied(true);
    } else {
      setIsFilterApplied(false);
    }
  }, [year, periodValueType, clientIdForFilter, statusTypes]);
  ...
  const valueSetter = () => {
    ...
    setYear(null);
    setClientName("");
    localStorage.removeItem("storedClientForInbox");
    setIsOpen(false);
    setPageIndexState(1);
  };
