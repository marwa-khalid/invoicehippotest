...
import { getClients } from "../client/client-search/core/_requests";
...
import { InvoicePostResult } from "../invoices/overview/core/_models";
import { ListLoading } from "./ListLoading";
interface Props {
  handleClose: any;
  formik: FormikProps<InvoicePostResult> | any;
  storageName: string;
}
const ClientSearch: FC<Props> = ({ handleClose, formik, storageName }) => {
  const intl = useIntl();

  const moduleKey = "client-picker";
  ...
  const setClient = async (client: any) => {
    if (formik != null) {
      ...
      localStorage.setItem(storageName, JSON.stringify(client));
    }
    handleClose();
  };
