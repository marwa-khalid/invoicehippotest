...
export function getInvoicesForPicker(
  searchTerm: string,
  pageIndex: number,
  dateRange: any,
  mutationId: number
) {
  return postRequest<InvoicePickerModel>(
    SEARCH_INVOICE_LIST,
    {
      pageMax: 5,
      pageIndex: searchTerm ? 1 : pageIndex,
...
export function getInvoiceViewData(invoiceId: number) {
  return getRequest<InvoiceViewDataModel>(
    `${INVOICE_API}/${invoiceId}/template/view-data`,
    true
  );
}
export function getQuickViewInvoice(invoiceId: number) {
  return getRequest<InvoiceListModel>(
    `${INVOICE_API}/quick-view-for/${invoiceId}`,
    true
  );
}
...
export function getAttachmentsForInvoice(invoiceId: number) {
