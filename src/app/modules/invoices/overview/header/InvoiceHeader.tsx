...
const InvoiceHeader = ({
  ...
  const [tempStatus, setTempStatus] = useState<any>(statusTypes);
  const [tempPeriodType, setTempPeriodType] = useState<number | null>(
    periodValueType
  );
  const [tempCreditInvoice, setTempCreditInvoice] = useState<boolean | null>(
    onlyCreditInvoices
  );
  ...
  useEffect(() => {
    if (clientIdForFilter) {
      setClientName(
        JSON.parse(localStorage.getItem("storedClientForInvoice")!)?.displayName
      );
    }
  }, [clientIdForFilter]);
  ...
  const valueSetter = () => {
    ...
    setTempStatus([]);
    setTempCreditInvoice(null);
    setOnlyCreditInvoices(null);
          ...
          `}
                ...
                <MenuList className="p-5 bg-body border-0 shadow-sm" zIndex={3}>
                  <InvoiceFilter
                    ...
                    tempCreditInvoice={tempCreditInvoice}
                    setTempCreditInvoice={setTempCreditInvoice}
