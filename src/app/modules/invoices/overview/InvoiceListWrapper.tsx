...
import { InvoiceList } from "./search-list/InvoiceList";
...
import { OrderConfirmationModal } from "./order-confirmation-modal/OrderConfirmationModal";
const InvoiceListInnerWrapper = ({
  ...
  const [onlyCreditInvoices, setOnlyCreditInvoices] = useState<boolean | null>(
    filters.creditFilter || null
  );
  ...
  const handleClientModalClose = () => {
    const storedClient = JSON.parse(
      localStorage.getItem("storedClientForInvoice")!
    );
  ...
  const fetchAttachments = async (id: number) => {
    const response = await getAttachmentsForInvoice(id);
  ...
  const attachFile = async (attachment: any) => {
    try {
      //check if fileid or some other attribute

      if (
        Object.entries(attachments).length > 0 &&
        attachments?.[editModalId].length === 5
      ) {
        toast.warning(
          intl.formatMessage({
        ...
        if (response.isValid) {
          ...
          const res = await getInvoiceById(editModalId);
          if (res.isValid) {
            setRefresh(!refresh);
          }
  ...
  return (
    <>
      <InvoiceHeader
        ...
        setOnlyCreditInvoices={setOnlyCreditInvoices}
        onlyCreditInvoices={onlyCreditInvoices}
      ...
      {showClientSearch && (
        <ClientSearch
          ...
          formik={null}
          storageName="storedClientForInvoice"
        />
      )}
      {copyModalOpen && (
        <InvoiceCopyModal
