...
import { finalizeInvoice, sendEmail } from "../core/_requests";
...
const InvoiceEmailModal = ({
  ...
  const formik = useFormik({
    initialValues: {
      invoiceId: invoiceId,
      overrideNotificationType:
        parsedData?.activeSendInstructions?.overrideNotificationType || 1,
      ...
      actionType: parsedData?.activeSendInstructions?.actionType || 1,
      adjustInvoiceDateToToday:
        parsedData?.activeSendInstructions?.adjustInvoiceDateToToday ?? false,
    },
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      try {
        ...
        let response2;
        if (response.isValid) {
          if (modalData.actionType === "send") {
            response2 = await finalizeInvoice(
              invoiceId,
              false,
              values.adjustInvoiceDateToToday
            );
          }
                ...
                <>
                  ...
                  <div className="form-check form-switch form-check-custom form-check-success form-check-solid mt-1 ms-2 d-flex align-items-center mb-10">
                    <input
                      ...
                      type="checkbox"
                      id="finalizeSwitch"
                      checked={formik.values.adjustInvoiceDateToToday}
                      onChange={(e) => {
                        formik.setFieldValue(
                          "adjustInvoiceDateToToday",
                          !formik.values.adjustInvoiceDateToToday
                        );
                        formik.setFieldValue(
                          "finalizeInvoice",
                          !formik.values.finalizeInvoice
                        );
                      }}
                    />
                    <label
                ...
                <>
                  ...
                  <div>
                    <h2 className="fw-normal mb-5">
                      {intl.formatMessage({
                        id: "Fields.ModalSendInvoiceOptionOverrideTemplate",
                      })}
                    </h2>
                    <Select
                      ...
                      value={getEnumOptions(
                        enums.InvoiceNotificationTypes,
                        intl
                      ).find(
                        (type) =>
