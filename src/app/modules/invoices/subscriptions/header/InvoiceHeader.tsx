...
const InvoiceHeader = ({
  ...
  const [tempSubscriptionStatus, setTempSubscriptionStatus] =
    useState<number>(subscriptionStatus);
  const [isFilterApplied, setIsFilterApplied] = useState<boolean>(false);
  useEffect(() => {
    if (clientIdForFilter) {
      setClientName(
        JSON.parse(localStorage.getItem("storedClientForSubscription")!)?.displayName
      );
    }
  }, [clientIdForFilter]);
