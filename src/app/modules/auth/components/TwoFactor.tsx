...
const TwoFactor = () => {
    ...
    onSubmit: async (values, { setStatus }) => {
      ...
      try {
        ...
        if (auth.isValid) {
          ...
          saveAuth(auth);
          if (auth.result.redirectToModule === 1) {
            navigate(
              `/invoice/view/${auth.result.redirectToModuleInstanceUniqueId}`
            );
          } else {
            navigate(
              `/estimation/view/${auth.result.redirectToModuleInstanceUniqueId}`
            );
          }
        }
  ...
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]); // Array of refs to focus inputs
