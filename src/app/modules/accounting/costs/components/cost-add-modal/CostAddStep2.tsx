...
import { CostItem, CostPostResult } from "../core/_models";
...
const CostAddStep2: FC<Props> = ({ formik, ledgers, vatTypes }) => {
  const intl = useIntl();
  const auth = useAuth();
  ...
  const [collapsedStates, setCollapsedStates] = useState<boolean[]>(
    formik.values.products.map((item) => !item.description)
  );

  useEffect(() => {
    // Sync collapse state if bookingItems change
    setCollapsedStates((prev) =>
      formik.values.products.map((item, index) =>
        typeof prev[index] !== "undefined" ? prev[index] : !item.description
      )
    );
      ...
      <form className="form" noValidate>
        <div className="d-flex justify-content-between align-items-center mb-5">
          ...
          <div className="d-flex position-relative align-items-center">
            <label
              className="required text-muted w-100 me-n18"
              htmlFor="amount"
            >
              {intl.formatMessage({
            ...
            <div className="custom-switch-vat ms-2 d-flex align-items-center flex-shrink-0">
              <input
              ...
              />
              <label
                className={`custom-label-vat py-2 text-nowrap ${
                  formik.values.header.validateTotalAmountWithoutVat
                    ? "bg-gray-200"
                    : "bg-primary"
                }`}
              ...
              >
                {formik.values.header.validateTotalAmountWithoutVat
                  ? intl.formatMessage({ id: "Fields.VatToggleLabelExc" })
                  : intl.formatMessage({ id: "Fields.VatToggleLabelInc" })}
              </label>
            </div>
          </div>
        </div>
                              ...
                              <td>
                                <div className="custom-switch-vat mt-1 ms-2 d-flex align-items-center mb-7 position-relative">
                                  ...
                                  >
                                    {costItem.totalPriceIsExVat
                                      ? intl.formatMessage({
                                          id: "Fields.VatToggleLabelExc",
                                        })
                                      : intl.formatMessage({
                                          id: "Fields.VatToggleLabelInc",
                                        })}
                                  </label>
                                </div>
                              </td>
