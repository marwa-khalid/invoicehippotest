...
import { BookingItem, BookingPostResult } from "../core/_models";
...
const BookingAddStep2: FC<Props> = ({ formik, ledgers, vatTypes }) => {
  ...
  const handleAddRow = (e: any, bookingItem: BookingItem) => {
    ...
    const newRow = {
      ...
      actions: {
        ...
        disableBalanceTypeEdit: false,
        disableLedgerAccountEdit: false,
        disableVatTypeEdit: false,
      },
    };
  ...
  const [collapsedStates, setCollapsedStates] = useState<boolean[]>(
    formik.values.bookingItems.map((item) => !item.description)
  );

  useEffect(() => {
    // Sync collapse state if bookingItems change
    setCollapsedStates((prev) =>
      formik.values.bookingItems.map((item, index) =>
                            ...
                            >
                              ...
                              >
                                {formik.values.bookingItems.length === 1 ? (
                                  ...
                                  <>
                                    {!bookingItem.actions.disableAmountEdit && (
                                      <button
