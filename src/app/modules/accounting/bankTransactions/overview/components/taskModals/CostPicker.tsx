...
import {
  CostListModel,
  CostListResult,
} from "../../../../costs/components/core/_models";
...
import { getStatusClass } from "../../../../costs/components/search-list/CostList";
...
import { getCostsForPicker } from "../../../../costs/components/core/_requests";
interface Props {
  handleClose: any;
  id: number;
  setLinkCostModalOpen: (type: boolean) => void;
  setCostData: (type: CostListResult) => void;
  setMutationId: (type: number) => void;
}
const CostPicker: FC<Props> = ({
  handleClose,
  id,
  setLinkCostModalOpen,
  setCostData,
  setMutationId,
}) => {
  const intl = useIntl();
  ...
  const [costs, setCosts] = useState<CostListModel>();
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const response = await getCostsForPicker(
        searchTerm,
        pageIndex,
        dateRange,
        id
        ...
        <div className="modal-dialog mw-800px">
          <div className="modal-content">
            <div className="modal-header bg-primary">
              <div className="fv-row col-12 d-flex justify-content-between align-items-center mb-0">
                <h2 className="fw-bolder mb-0 text-white">
                  {intl.formatMessage({
                    id: "Fields.EditPanelTitleReceipts",
                  })}
                </h2>
                <div
            ...
            <div className="modal-body bg-secondary">
              ...
              {costs && costs.totalRows > 0 && (
                <h5 className="px-2 mb-10">
                  {intl
                    .formatMessage({ id: "Fields.SearchResultHeaderCount" })
                    .replace("{0}", costs?.totalRows.toString())}
                </h5>
              )}

              <div className="row row-cols-1 row-cols-md-1 g-4">
                {costs?.result?.map((cost: CostListResult) => (
                  <ListCard key={cost.id}>
                    {/* {cost.hasVoucherNr && (
                      <div className="ms-15 ">
                          ...
                          <div className="table" style={{ width: "100%" }}>
                            ...
                            {cost.totals.totalPrice && (
                              <div style={{ display: "table-row" }}>
                                ...
                                >
                                  {cost.valuta.sign}
                                  {cost.totals.totalPrice.toFixed(2)}
                                </div>
                              </div>
                            )}
                            {/* Total VAT Amount */}
                            {cost.totals.totalVATAmount !== 0 && (
                              <div style={{ display: "table-row", gap: 3 }}>
                                ...
                                >
                                  {cost.valuta.sign}
                                  {cost.totals.totalVATAmount.toFixed(2)}
                                </div>
                              </div>
                            )}
                          </div>
                    ...
                    >
                      ...
                      >
                        <div className="ribbon-label fw-bold">
                          {cost.valuta.sign}
                          {cost.totals.totalPriceWithVAT.toFixed(2)}
                              ...
                              <span className="fs-sm text-muted d-flex align-items-center position-relative pe-6">
                                ...
                                {cost.hasAttachments && (
                                  <span className="position-absolute bottom-60 start-100 translate-middle badge badge-sm badge-square badge-primary fs-8 p-2">
                                    ...
                                    />
                                    {cost.attachmentsCount}
                                  </span>
                                )}
                              </span>

                              <span className="fw-bolder text-primary">
                                {cost.invoiceDateAsString}
                              </span>
                            </div>
                          </div>
                          <span
                          ...
                          <div className="d-flex flex-column">
                            ...
                            <ul className="breadcrumb breadcrumb-secondary breadcrumb-dot my-1 text-muted">
                              <li className="breadcrumb-item">
                                {cost.invoiceStatus.name && (
                                  <small
                                    className={`${getStatusClass(
                                      cost.invoiceStatus.value
                ...
                {costs?.result?.length == 0 && <NoItemsPage />}
              ...
              <div className="flex-grow-1">
                {costs && costs.totalRows > 0 && (
                  <ListPagination
                    ...
                    pageIndex={costs?.pageIndex}
                    onPageChange={handlePageChange}
                    totalItems={costs?.totalRows}
