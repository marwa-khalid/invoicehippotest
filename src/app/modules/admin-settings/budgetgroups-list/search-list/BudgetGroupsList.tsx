...
import { getBudgetGroups } from "../core/_requests";
import { BudgetGroupResult } from "../core/_models";
...
const BudgetGroupsList = ({
  ...
  const fetchProductGroups = async () => {
    setIsLoading(true);
    try {
      const response = await getBudgetGroups(searchTerm, pageIndex);
      if (response.isValid) {
  ...
  const toggleAllRowsSelection = () => {
    if (selectAll) {
      ...
      const allIds = productGroups.result.map(
        (item: BudgetGroupResult) => item.id
      );
