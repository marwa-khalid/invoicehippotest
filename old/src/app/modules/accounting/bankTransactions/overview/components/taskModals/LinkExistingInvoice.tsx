...
interface Props {
  handleClose: any;
  invoiceData: InvoiceListResult | null;
  setShowInvoices: (type: boolean) => void;
  setInvoiceData: (type: InvoiceListResult | null) => void;
  mutationId: number;
  financialAccounts: AccountsResult[];
}
const LinkExistingInvoice: FC<Props> = ({
  handleClose,
  invoiceData,
  setShowInvoices,
  setInvoiceData,
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      if (invoiceData) {
        const res = await getTransferOptionsInvoice(invoiceData.id, mutationId);
        if (res.isValid) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await registerPaymentInvoice(values);
        if (response.isValid) {
          handleClose();
        }
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              <div className="col-7 mt-5">
                ...
                <InvoiceAddButtons
                  setInvoiceSearch={setShowInvoices}
                  data={invoiceData}
                  reset={reset}
                />
              </div>
                ...
                <table className="table mt-0">
                  <tbody>
                    <tr className="my-0">
                      ...
                      <td>: {invoiceData?.invoiceStatus.description}</td>
                    </tr>
                    <tr>
                      ...
                      <td>: {invoiceData?.invoiceDateAsString}</td>
                    </tr>
                    {invoiceData?.client && (
                      <tr>
                        <td className="fw-bold">
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        {invoiceData?.totals.totalPriceWithVAT.toFixed(2)}
                      </td>
                    </tr>
                    <tr>
                      <td className="fw-bold"></td>
                ...
                !transferOptions?.restAmountOptions
                  ...
                  <div className="form-check form-switch form-check-custom form-check-success form-check-solid mt-1 ms-2 d-flex align-items-center mb-7">
                    <input
                      ...
                      type="checkbox"
                      id="fullOrPartialSwitch"
                      checked={formik.values.residualAmountActionType === null}
                      onChange={(e) => {
                        formik.setFieldValue("residualAmountActionType", null);
                      }}
                    />
