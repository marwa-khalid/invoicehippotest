...
const CustomFieldsList = ({
  ...
  const toggleAllRowsSelection = () => {
    if (selectAll) {
      ...
      const allIds = customFields.result.map(
  ...
  return (
    <div className="card py-3  p-10">
      ...
      {customFields?.result?.length > 0 && (
        <ListPagination
          totalPages={customFields.totalPages}
          pageIndex={pageIndex}
          moduleName="customfields-module"
          onPageChange={handlePageChange}
          totalItems={customFields.totalRows}
        />
      )}
    </div>
  );
