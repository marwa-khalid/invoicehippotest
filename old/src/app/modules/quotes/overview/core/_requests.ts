...
import { PRODUCT_API } from "../../../products/products-list/core/constants";
import { AttachmentsModel } from "../../../accounting/bookings/components/core/_models";
import { GenericNumberModel } from "../../../invoices/overview/core/_models";
...
export function getQuoteViewData(quoteId: number) {
  return getRequest<QuoteViewDataModel>(
    `${QUOTE_API}/${quoteId}/template/view-data`,
    true
  );
}
...
export function getProductsForPicker(
  searchTerm: string,
  pageIndex: number,
  pageMax: number
) {
...
export function getAttachmentsById(quoteId: number) {
  return getRequest<AttachmentsModel>(`${GET_ATTACHMENTS}/${quoteId}`, true);
}

export function attachQuoteFile(
  inboxItemId: number,
  fileId: number,
  quoteId: number
) {
  return postRequest<GenericBooleanModel>(
    `${QUOTE_ATTACHMENT}/${quoteId}`,
    {
      inboxItemId,
      fileId,
    },
...
export function getInboxAttachments(
  ...
  return postRequest<any>(
    INBOX_SEARCH,
    {
      ...
      searchTerm: searchTerm,
      pageIndex: pageIndex,
      fileTypes: fileTypes ? fileTypes : [1, 2, 3],
    },
    true
  );
}
export function createOrderConfirmation(values: any) {
...
export function getEstimationActivitiesById(id: number) {
  return getRequest<ActivitiesModel>(`${ESTIMATION_ACTIVITIES}/${id}`, true);
}
