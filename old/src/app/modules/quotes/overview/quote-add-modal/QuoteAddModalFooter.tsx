...
const QuoteAddModalFooter = ({
  ...
  const valueSetter = async (actionType: string) => {
    const response = await getQuickViewQuote(formik.values.id);
    if (response.isValid) {
      setEditModalId(response.result.id);
      setQuoteNr(response.result.quoteNr);
      localStorage.setItem(
        "ModalData",
        JSON.stringify({
          quoteDateAsString: response?.result.quoteDateAsString,
          ...
          status: response?.result.quoteStatus.value,
