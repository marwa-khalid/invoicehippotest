...
interface ComponentProps {
  ...
  setSubscriptionStatus: (type: number) => void;
  setStatusTypes: (types: any) => void;
  setTempSubscriptionStatus: (type: number) => void;
  tempSubscriptionStatus: number;
  tempStatus: any;
...
export function InvoiceFilter({
  ...
  const resetClient = () => {
    ...
    localStorage.removeItem("storedClientForSubscription");
  };
  const quarterList = [
  ...
  const handleCreditTypeChange = (selectedOption: any) => {
    setTempSubscriptionStatus(selectedOption ? selectedOption.value : null);
  };
  ...
  const handleApply = () => {
    ...
    // Update the pagination state for the "subscription-module"
    ...
    updatePagination(
      moduleKey,
      {
        ...
        subscriptionTypeFilter: tempSubscriptionStatus || null,
      ...
      <div className="px-7 py-5">
              ...
              enums.InvoiceStatusTypes.filter(
                (item) =>
                  currentUser?.result.isAnonymousUser
                    ? [128, 256].includes(item.Value) // If anonymous, only 128 and 256
                    : [1, 128, 256].includes(item.Value) // Otherwise, allow 1, 128, and 256
              ),
              intl
            )}
          />
        ...
        <div className="mb-5">
          <label className="form-label fw-bold" htmlFor="credit">
            {intl.formatMessage({
              id: "Fields.SelectOptionFilterOnInvoiceSubscriptionStatusType",
            })}
            :
          </label>
          <Select
            ...
            menuPlacement="bottom"
            value={
              getEnumOptions(enums.InvoiceSubscriptionStatusTypes, intl).find(
                (option) => option.value === tempSubscriptionStatus
              ) || null
            }
            inputId="credit"
            onChange={handleCreditTypeChange}
            options={getEnumOptions(enums.InvoiceSubscriptionStatusTypes, intl)}
            placeholder={intl.formatMessage({
